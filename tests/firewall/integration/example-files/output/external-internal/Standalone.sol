...
contract Standalone is FirewallConsumer {
    /**
        Some
        Multi-line
        comment.
    ...
    function externalOverrideFunction(bytes calldata param) external override firewallProtected {
        // some code
    }
    ...
    ) external override firewallProtected returns (bool res1, bytes memory res2) {
    ...
    function internalFunctionComplexTypes(
        ...
        address payable _payableAddressVar,
        uint256 _storageVar
    ) internal firewallProtectedSig(bytes4(0x9b4e77d1)) {
    ...
    ) internal firewallProtectedSig(bytes4(0x966f2245)) {
