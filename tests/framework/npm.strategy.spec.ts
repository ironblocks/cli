...
describe('NPM Strategy', () => {
    ...
    beforeEach(async () => {
        ...
        const { unit, unitRef } = TestBed.create(NPMStrategy).compile();
    ...
    describe('.isDependencyInstalled()', () => {
        it('uses "npm ls" to check if the dependency is installed', async () => {
            ...
            await npmStrategy.isDependencyInstalled(mockDependency);

            const executedCommand = (exec as unknown as jest.Mock).mock.calls[0][0];
            expect(executedCommand).toContain(`npm ls "${mockDependency.installName}"`);
        });
        ...
        it('returns true if the dependency is installed', async () => {
            ...
            const isInstalled = await npmStrategy.isDependencyInstalled(mockDependency);
    ...
    describe('.installDependency()', () => {
        it('logs a spinner while installing the dependency', async () => {
            ...
            await npmStrategy.installDependency(mockDependency);

            expect(loggerMock.spinner).toHaveBeenCalledWith(`Installing "${colors.cyan(mockDependency.name)}"`);
        });

        it('installs the dependency using "npm install"', async () => {
            (exec as unknown as jest.Mock).mockImplementation((cmd, options, callback) => {
                callback(null, { stdout: 'some output indicating success', stderr: '' });
            });
            ...
            const executedCommand = (exec as unknown as jest.Mock).mock.calls[0][0];
            expect(executedCommand).toContain(`npm install`);
        });
