...
export class LiveService {
    ...
    /**
     * Switches Venn integration to Live mode
     */
    async switchToLive(options: LiveModeOptions): Promise<void> {
        ...
        await this.disableDryRun(contracts, networkConfig, wallet, provider);
    }

    private async disableDryRun(
        ...
        for (const contract of contracts) {
            ...
            if (!contract.isDryRunEnabled) {
                this.logger.log(` -> Contract ${colors.cyan(contract.name)} is already in live mode. Skipping...`);
                continue;
            }

            this.logger.log(` -> Setting live mode for contract ${colors.cyan(contract.name)}`);

            const tx = await firewall.setConsumerDryrunStatus(contract.address, false);
        ...
        //
        this.logger.success(` -> Live mode enabled for all contracts!`);
    }
